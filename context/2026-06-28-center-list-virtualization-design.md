...
## 향후 개선 방향 — 진짜 서버 사이드 count + status별 list (데이터 대량화 시)

현재 구현은 **카운트/필터를 클라이언트에서 처리**한다. 즉 센터 전체를 경량 index로 한 번
불러온 뒤, 진행/대기/완결 개수는 그 배열을 `reduce`하고 리스트는 `filter`한다. 카운트가
"렌더된 카드 수"가 아니라 전체 로드된 index 기준이라 정확하지만, 사용자가 원래 떠올린
`SELECT COUNT(*) WHERE status=...` + `SELECT * WHERE status=... LIMIT/OFFSET` 형태의
**서버 쿼리는 아니다**. (참고: 실제로 character에 `status` 컬럼은 없고 진행/대기/완결은
대화 집계로 파생 — 서버 count도 단순 COUNT가 아니라 집계가 필요하다.)

한 센터가 **수만 개 이상으로 커지면** 경량 index 1회 로드(수백 KB~)가 부담이 되므로
아래 서버 방식으로 전환을 검토한다:

- **카운트 전용 엔드포인트**: 대화 집계를 status로 묶어 `{active, waiting, completed}`
  숫자만 반환(전체 행 미반환).
- **status별 페이지네이션 리스트 엔드포인트**: 파생 status로 필터 + LIMIT/OFFSET(또는 cursor).
- **트레이드오프**: 이 경우 태그/성별/검색/랜덤정렬·뒤로가기 즉시복원도 서버로 옮기거나
  제약이 생긴다. 현재 클라 방식은 그 기능들이 전부 즉시 동작하는 게 장점이므로,
  전환 시 어디까지 서버로 옮길지 함께 설계해야 한다.

전환 트리거(권장): 한 센터 항목 수가 대략 **1만 개를 넘거나** index 로드 체감 지연이
생길 때. 그 전까진 현 구조 유지.
