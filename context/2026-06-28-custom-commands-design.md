...
- **지시문 작성 가이드(`?` 도움말)**: 지시문 입력 필드 제목 옆에 `?` 아이콘 → 클릭 시 짧은 가이드 토글
  (툴팁/접이식 패널). 내용:
  - **무엇을·어떤 형식으로**: "현재 상황을 확인해 ○○을 ××형식으로 작성하라"처럼 행동 + 출력 형식을 명시.
  - **출력 모양 지정**: 에브리타임 게시판/채팅 로그/표 등 원하는 형태를 직접 묘사("게시글 제목·본문·댓글 형식").
  - **마크다운**: "마크다운으로 작성"이라 적으면 헤딩·목록·구분선 등으로 예쁘게 렌더됨(시스템이 마크다운 출력을 한 번 더 보강함).
  - **맥락은 자동**: 현재 대화·캐릭터·상황을 AI가 알아서 참고하므로 따로 붙여넣을 필요 없음. `!이름 뒤 텍스트`로 그때그때 추가 지시 가능.
  - **예시**: `!에타` → "지금까지의 상황을 에브리타임 자유게시판 글처럼, 제목 + 본문 + 익명 댓글 몇 개를 마크다운으로 작성하라."
  - 가이드 문구는 컴포넌트 상수로 두고, 추후 `guide/page.tsx`와 일관 유지.
